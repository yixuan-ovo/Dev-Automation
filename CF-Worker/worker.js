...
var src_default = {
  async fetch(request, env) {
    ...
    if (urlParam.startsWith("proxies:")) {
      ...
      for (const url2 of urlParts) {
        ...
        if (url2.startsWith("https://") || url2.startsWith("http://")) {
          try {
            ...
            try {
              ...
              // 简易判断常见本地/私网 IPv6 前缀：::1, fc/fd, fe80
              const isIPv6Local = isIPv6 && (/^(::1)$/i.test(host) || /^(fc|fd|fe80)/i.test(host));
              const isIPAddress = isIPv4 || isIPv6Local || isLocalhost;

              // 仅当目标是 IP/本地时才走 relay
              if (isIPAddress) {
                const relayUrl = `https://relay.yixr.uno/?target=${encodeURIComponent(urlToFetch)}`;
                console.log(`Detected IP source, using relay: ${relayUrl}`);
                response = await fetch(relayUrl, fetchOptions);
              } else {
                // 域名直接 fetch，不走 relay
                response = await fetch(urlToFetch, fetchOptions);
              }

              clearTimeout(timeoutId);
            } catch (err) {
              clearTimeout(timeoutId);
              console.error(`Fetch error for ${urlToFetch}:`, err);
              throw err;
            }

            console.log(`Response status: ${response.status}`);
            if (!response.ok) {
              console.error(`Failed to fetch ${url2}, status: ${response.status}`);
              continue;
            }
            const plaintextData = await response.text();
            console.log(`Fetched data length: ${plaintextData.length}, first 200 chars: ${plaintextData.substring(0, 200)}`);
            parsedObj = parseData(plaintextData);
            console.log(`Parsed format: ${parsedObj.format}`);
            // 仅白名单必要头部，避免敏感或冗余头
            ...
            // 单个订阅失败不影响其他订阅
            console.error(`Failed to fetch ${url2}:`, fetchError.message);
        ...
        if (/^(ssr?|vmess1?|trojan|vless|hysteria):\/\//.test(url2)) {
          ...
          console.error(`Unknown format for URL ${url2}: format=${parsedObj.format}`);
          // 对于未知格式，检查是否是纯文本节点链接
          const data = typeof parsedObj.data === 'string' ? parsedObj.data : JSON.stringify(parsedObj.data);
          if (data && data.trim().length > 0) {
            // 尝试将其作为纯文本节点链接处理（每行一个）
            const links = data.split(/\r?\n/).filter((link) => link.trim() !== "");
            if (links.length > 0 && /^(ssr?|vmess1?|trojan|vless|hysteria):\/\//.test(links[0])) {
              // 这看起来是多行节点链接
              const newLinks = [];
              for (const link of links) {
                const newLink = replaceInUri(link, replacements, false);
              ...
              if (newLinks.length > 0) {
                const replacedData = newLinks.join("\r\n");
                await SUB_BUCKET.put(key, replacedData);
                keys.push(key);
                replacedURIs.push(`${host}/${subDir}/${key}`);
              }
            } else {
              // 原样存储
              await SUB_BUCKET.put(key, data);
              keys.push(key);
              replacedURIs.push(`${host}/${subDir}/${key}`);
            }
          }
    ...
    console.log(`Processed ${replacedURIs.length} subscription links`);
    const newUrl = replacedURIs.join("|");
    console.log('New URL:', newUrl);
    if (!newUrl) {
      return new Response("Failed to fetch any subscription links or no valid links found", { status: 400 });
    }
    // 保留所有原始参数并更新 url 参数
    const searchParams = new URLSearchParams(url.search);
    searchParams.set("url", newUrl);
    // 移除 bd 参数（如果有），因为后端地址已经确定
    searchParams.delete("bd");
    const backendUrl = backend + url.pathname + "?" + searchParams.toString();
    console.log('Backend URL:', backendUrl);
    const modifiedRequest = new Request(backendUrl, request);
    const rpResponse = await fetch(modifiedRequest);
    console.log('Backend response status:', rpResponse.status);
    ...
    if (rpResponse.status === 200) {
      ...
      try {
        ...
        if (keys.length === 0) {
          return new Response(plaintextData, rpResponse);
        }
...
function parseData(data) {
  // 优先识别明显的 YAML
  try {
    if ((/(^|\n)\s*proxies\s*:/).test(data)) {
      return { format: "yaml", data: yaml.load(data) };
    }
  } catch (_) {}

  // 检查是否可能是 Base64（URL-safe 或标准）
  const cleaned = data.replace(/\s/g, "");
  const isUrlSafeB64 = /^[A-Za-z0-9\-_]+={0,2}$/.test(cleaned) && cleaned.length % 4 === 0;
  const isStdB64 = /^[A-Za-z0-9\+/]+={0,2}$/.test(cleaned) && cleaned.length % 4 === 0;
  if (isUrlSafeB64) {
    try {
      return { format: "base64", data: urlSafeBase64Decode(cleaned) };
    } catch (_) {}
  }
  if (isStdB64) {
    try {
      return { format: "base64", data: atob(cleaned) };
    } catch (_) {}
  }
